...
class MoodleClient(object):
    ...
    def getUserData(self):
        tokenUrl = self.path+'login/token.php?service=moodle_mobile_app&username=' + \
    ...
    def getDirectUrl(self, url):
        ...
        direct = self.path+'webservice/pluginfile.php/' + \
            tokens[4]+'/user/private/'+tokens[-1]+'?token='+self.data['token']
        return direct
    ...
    def login(self):
        ...
        if counter > 0:
            ...
            self.userid = soup.find(
                'div', {'id': 'nav-notification-popover-container'})['data-userid']
    ...
    def createEvidence(self, name, desc=''):
        ...
        saveevidence = self.path + \
            'admin/tool/lp/user_evidence_edit.php?id=&userid='+self.userid+'&return='
        ...
        print('Evidencia creado con exito')

        return {'name': name, 'desc': desc, 'id': evidenceid, 'url': resp.url, 'files': []}

    def saveEvidence(self, evidence):
        evidenceurl = self.path + 'admin/tool/lp/user_evidence_edit.php?id=' + \
            evidence['id']+'&userid='+self.userid+'&return=list'
        resp = self.session.get(evidenceurl)
        soup = BeautifulSoup(resp.text, 'html.parser')
        sesskey = soup.find('input', attrs={'name': 'sesskey'})['value']
        files = evidence['files']
        saveevidence = self.path + 'admin/tool/lp/user_evidence_edit.php?id=' + \
            evidence['id']+'&userid='+self.userid+'&return=list'
        payload = {'userid': self.userid,
                'sesskey': sesskey,
                '_qf__tool_lp_form_user_evidence': 1,
                ...
                'submitbutton': 'Guardar+cambios'}
        resp = self.session.post(saveevidence, data=payload)
        print('Evidencia guardado con exito')
        return evidence
    ...
    def getEvidences(self):
        ...
        for n in nodes:
            ...
            nfilelist = []
            for f in nodefiles:
                directurl = str(f['href']).replace(
                    'pluginfile.php', 'webservice/pluginfile.php') + '&token=' + self.userdata['token']
                nfilelist.append({'name': f.next, 'url': directurl})
            list.append({'name': evname, 'desc': '', 'id': evid,
                        'url': evurl, 'files': nfilelist})
            print('Evidencia cargada con exito')
    ...
    def deleteEvidence(self, evidence):
        ...
        soup = BeautifulSoup(resp.text, 'html.parser')
        sesskey = soup.find('input', attrs={'name': 'sesskey'})['value']
        deleteUrl = self.path+'lib/ajax/service.php?sesskey='+sesskey + \
            '&info=core_competency_delete_user_evidence,tool_lp_data_for_user_evidence_list_page'
    ...
    def upload_file(self, file, evidence, itemid=None):
        ...
        query = self.extractQuery(
            soup.find('object', attrs={'type': 'text/html'})['data'])
        ...
        resp2 = self.session.post(
            post_file_url, files=upload_file, data=upload_data)
        of.close()
    ...
    def getFiles(self):
        ...
        query = self.extractQuery(
            soup.find('object', attrs={'type': 'text/html'})['data'])
        payload = {'sesskey': sesskey, 'client_id': client_id,
                'filepath': filepath, 'itemid': query['itemid']}
    ...
    def delteFile(self, name):
        ...
        resp = self.session.get(urlfiles)
        soup = BeautifulSoup(resp.text, 'html.parser')
        _qf__core_user_form_private_files = soup.find(
            'input', {'name': '_qf__core_user_form_private_files'})['value']
        files_filemanager = soup.find(
            'input', attrs={'name': 'files_filemanager'})['value']
        sesskey = soup.find('input', attrs={'name': 'sesskey'})['value']
        client_id = self.getclientid(resp.text)
        filepath = '/'
        query = self.extractQuery(
            soup.find('object', attrs={'type': 'text/html'})['data'])
        payload = {'sesskey': sesskey, 'client_id': client_id,
                'filepath': filepath, 'itemid': query['itemid'], 'filename': name}
        postdelete = self.path+'repository/draftfiles_ajax.php?action=delete'
        resp = self.session.post(postdelete, data=payload)

        # save file
        saveUrl = self.path+'lib/ajax/service.php?sesskey=' + \
            sesskey+'&info=core_form_dynamic_form'
        savejson = [{"index": 0, "methodname": "core_form_dynamic_form", "args": {"formdata": "sesskey="+sesskey+"&_qf__core_user_form_private_files=" +
                    _qf__core_user_form_private_files+"&files_filemanager="+query['itemid']+"", "form":"core_user\\form\\private_files"}}]
        headers = {'Content-type': 'application/json',
                   'Accept': 'application/json, text/javascript, */*; q=0.01'}
